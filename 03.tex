...
% Let $G$ be a finite subgroup of $S^1$. Let $n=|G|$. 
% We claim that 
% $G\subseteq\{\exp(2\pi ik/n):0\leq k\leq n-1\}$. Let $g\in G$. 
% Since $g^n=1$, $g$ is a $n$-th root of one, say 
% $g=\exp(2\pi i k/n)$ for some $k\in\{1,\dots,n-1\}$. Now the 
% claim follows, since $\{\exp(2\pi ik/n):0\leq k\leq n-1\}$ is
% cyclic. 
