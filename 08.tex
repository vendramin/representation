...
\begin{proof}
    ...
    \]
    and hence $(\chi(g)-1)(\chi(g)-n)=0$
    for all $g\in G\setminus G_0$. 
    
    Let $X=\{1,\dots,n\}$. By assumption, $G$ 
    acts transitively on $X$. Let 
    $\chi$ be the character of $G$ 
    given by $\chi(g)=|\Fix(g)|$ and $G_0$ be the 
    set of derangements of $G$. By Corollary~\ref{cor:Frobenius_combinatorio}, 
    the subgroup $N=G_0\cup\{\id\}$ is normal in $G$. Let $x\in X$ and $H=G_x$. Then... 
