...
\begin{proof}
    ...
    $\chi=\Ind_I^G\psi_1=\Ind_I^G\psi_2\in\Irr(G)$. We want to prove that $\psi_1=\psi_2$. 
    
    Suppose $\psi_1\ne\psi_2$. 
    We know that $\psi_1$ and $\psi_2$ are constituents
    of $\Res_I^G\chi$, that is 
    \[
    \Res_I^G\chi=\psi_1+\psi_2+\xi 
    \]
    for some map $\xi\colon G\to\C$. 
    Then both $\Res_N^I\psi_1$ and 
    $\Res_N^I\psi_2$ are constituents 
    of 
    $\Res_N^G\chi=\Res_N^I\Res_I^G\chi$. Thus 
    \[
    \Res_N^G\chi=\Res_N^I\psi_1+\Res_N^I\psi_2+\xi
    \]
    where the dots represent either
    the zero map or some character of $N$. 
    Moreover, 
    \begin{align*}
        \langle\Res_N^G\chi,\theta\rangle &= \frac{1}{|N|}\sum_{n\in N}(\Res_N^G\chi)(n)\overline{\theta(n)}\\
        &=\frac{1}{|N|}\sum_{n\in N}(\Res_N^I\psi_1)(n)+(\Res_N^I\psi_2)(n)+\cdots)\overline{\theta(n)}\\
