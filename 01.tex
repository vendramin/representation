...
Clearly, any finite direct sum of semisimples is semisimple. 

\begin{exercise}
    Let $M$ be a finite-dimensional module. Prove that $M$ is semisimple 
...
\]
It turns out that $J(A)$ is an ideal of $A$. 
...
The ideals of $\prod_{i=1}^k M_{n_i}(\C)$ 
are of the form $I_1\times\cdots\times I_k$, where
each $I_j$ is a either zero or $M_{n_j}(\C)$. 
...
The ideas behind the theorem are somewhat connected to Sylow theory. 

Let study explicitly  
Sylow theorems for the group $\GL_n(p)$ of invertible matrices 
...
\begin{proposition}
Let $P$ be a finite $p$-group acting on an $\F_p$-vector space 
$V$ by automorphisms. Then there exists a non-zero $v\in V$ 
such that $P\cdot v=v$. 
\end{proposition}
...
\begin{proposition}
    The normalizer of $U_n(p)$ in $\GL_n(p)$ is the
...
\begin{theorem}
    The group $U_n(p)$ is a Sylow $p$-subgroup of $\GL_n(p)$. 
\end{theorem}
...
The third Sylow's theorem...

\begin{theorem}
    Let $n_p$ be the number of conjugates of $U_n(p)$ in $\GL_n(p)$. 
    Then $n_p\equiv1\bmod p$. 
\end{theorem}
